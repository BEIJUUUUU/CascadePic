...
[✨ 核心特性](#-核心特性) • [📸 界面预览](#-界面预览) • [📥 下载与安装](#-下载与安装) • [🖱️ 右键集成](#️-资源管理器右键集成) • [⌨️ 快捷键指南](#️-快捷键指南) • [🛠️ 本地构建](#️-本地构建)
...
- 🖱️ **资源管理器右键集成**：一键把「用流瀑看图打开」加进文件夹与图片右键菜单（免管理员权限）。
...
无需管理员权限、不弹 UAC，即可把「用流瀑看图打开」加入 Windows 右键菜单。

### 一键启用
双击 **`安装右键菜单.cmd`**，或在程序内点击工具栏的 **右键集成** 按钮。
...
| :--- | :--- |
| **文件夹上右键** | 直接以瀑布流打开该文件夹 |
| **文件夹空白处右键** | 直接以瀑布流打开当前文件夹 |
| **图片 / 视频文件上右键** | 打开该文件并自动定位到同目录瀑布流 |
| **「打开方式」候选列表** | 出现在系统的“打开方式”列表中 |

支持的文件类型：`jpg` `jpeg` `jfif` `png` `bmp` `gif` `webp` `tif` `tiff` `ico` `mp4` `mkv` `webm` `mov` `avi` `wmv` `m4v`

### 命令行启用（适合脚本化部署）
```powershell
# 启用
.\dist\CascadePic\CascadePic.exe --install-context-menu

# 移除
.\dist\CascadePic\CascadePic.exe --uninstall-context-menu
```

### 一键移除
双击 **`卸载右键菜单.cmd`**，或在程序内再次点击工具栏的 **右键集成** 按钮取消勾选。
注册信息全部写在 `HKEY_CURRENT_USER` 下，卸载后可完整还原，不留残余。
